...
case "$RATIO" in
  5)
    ...
    NAME_PREFIX="fixmatch_ifcf_bus_ld10_ratio5"
    METHOD_SUFFIX="fixmatch_ifcf_bus_ld10_ratio5"
    SUMMARY="results/fixmatch_ifcf_bus_ld10_ratio5_latest.csv"
    ;;
  *)
...
LOG=${LOG:-logs/fixmatch_ifcf_bus_ld10_ratio${RATIO}_${STAMP}.log}
...
echo "[$(date)] ===== FixMatch+IFCF BUS ld=1.0 ratio=${RATIO}% start ====="
echo "base=${BASE}"
...
touch "results/ALL_DONE_FIXMATCH_IFCF_BUS_LD10_RATIO${RATIO}"
echo "[$(date)] ===== FixMatch+IFCF BUS ld=1.0 ratio=${RATIO}% done ====="
