...
TEMPS=${TEMPS:-"0.1 0.3 0.5 0.7"}
STAMP=$(date +%Y%m%d_%H%M%S)
LOG=${LOG:-logs/bus_fixmatch_ifcf_ld1_temp_scan_${STAMP}.log}
BASE=${BASE:-config/usb_cv/fixmatch/fixmatch_bus_878_0.yaml}
SUMMARY=${SUMMARY:-results/fixmatch_ifcf_bus_ld1_temp_scan_latest.csv}
...
run_temp() {
  local temp=$1
  local temp_key
  temp_key=$(temp_tag "$temp")
  local tag="fixmatch_ifcf_bus_ld1_hard_True_wu1_w1_ct${temp_key}"
...
echo "[$(date)] ===== BUS FixMatch+IFCF ld=1.0 temperature scan start ====="
echo "base=${BASE}"
...
touch results/ALL_DONE_BUS_FIXMATCH_IFCF_LD1_TEMP_SCAN
echo "[$(date)] ===== BUS FixMatch+IFCF ld=1.0 temperature scan done ====="
