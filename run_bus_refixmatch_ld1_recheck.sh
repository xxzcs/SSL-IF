...
LOG=${LOG:-logs/bus_refixmatch_ld1_recheck_${STAMP}.log}
SUMMARY=${SUMMARY:-results/refixmatch_bus_ld1_recheck_latest.csv}
BASE=${BASE:-config/usb_cv/refixmatch/refixmatch_bus_878_0.yaml}
...
echo "[$(date)] ===== ReFixMatch BUS ld=1.0 recheck start ====="
echo "base=${BASE}"
...
for s in $SEEDS; do
  sn="refixmatch_bus_ld1_recheck_s${s}"
  ...
  cp "$BASE" "$tmp"
  setkv "$tmp" algorithm refixmatch
...
python3 eval_sup.py \
  ...
  --load_glob "saved_models/usb_cv/refixmatch_bus_ld1_recheck_s*/latest_model.pth" \
  --method_suffix "refixmatch_bus_ld1_recheck_latest"

touch results/ALL_DONE_BUS_REFIXMATCH_LD1_RECHECK
echo "[$(date)] ===== ReFixMatch BUS ld=1.0 recheck done ====="
