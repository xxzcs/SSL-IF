...
LOG=${LOG:-logs/bus_fixmatch_ifcf_ld1_phase1_${STAMP}.log}
BASE=${BASE:-config/usb_cv/fixmatch/fixmatch_bus_878_0.yaml}
SUMMARY=${SUMMARY:-results/fixmatch_ifcf_bus_ld1_hardstrong_wu510_latest.csv}
...
run_combo() {
  ...
  local strong=$2
  local warmup=$3
  local tag="fixmatch_ifcf_bus_ld1_${target}_${strong}_wu${warmup}"
  ...
  for s in $SEEDS; do
    ...
    setkv "$tmp" corrT 0.9
    setkv "$tmp" use_strong_if "$strong"
...
echo "[$(date)] ===== BUS FixMatch+IFCF ld=1.0 hard-strong warmup5/10 start ====="
echo "base=${BASE}"
...
run_combo hard True 5
run_combo hard True 10

touch results/ALL_DONE_BUS_FIXMATCH_IFCF_LD1_HARDSTRONG_WU510
echo "[$(date)] ===== BUS FixMatch+IFCF ld=1.0 hard-strong warmup5/10 done ====="
