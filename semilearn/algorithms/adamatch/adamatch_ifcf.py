# AdaMatch + 闭式 last-layer TracIn 影响排序一致性 (纯 add; AdaMatch 无关系项)。
# 不修改原 adamatch.py。train_step 忠实复刻 AdaMatch + 追加 ifrank_loss(软目标 mixin)。
import torch
...
@ALGORITHMS.register('adamatch_ifcf')
class AdaMatchIFcf(ClosedFormIFRankMixin, AdaMatch):
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s):
        ...
        with self.amp_cm():
            ...
            mask = self.call_hook("masking", "MaskingHook", logits_x_lb=probs_x_lb, logits_x_ulb=probs_x_ulb_w,
                                  softmax_x_lb=False, softmax_x_ulb=False)
    ...
    @staticmethod
    def get_argument():
        return AdaMatch.get_argument() + ClosedFormIFRankMixin.ifrank_arguments()
