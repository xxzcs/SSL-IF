# ReFixMatch + 闭式 last-layer TracIn 影响排序一致性 (纯 add)。
# 不修改原 refixmatch.py。train_step 逐字复刻 ReFixMatch(含其软 KL 项 unsup_loss_refix)+ 追加 ifrank_loss。
import torch
...
@ALGORITHMS.register('refixmatch_ifcf')
class ReFixMatchIFcf(ClosedFormIFRankMixin, ReFixMatch):
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s):
        ...
        with self.amp_cm():
            ...
            pseudo_label = self.call_hook("gen_ulb_targets", "PseudoLabelingHook",
                                          logits=probs_x_ulb_w, use_hard_label=True,
                                          T=self.T, softmax=False)

            # ReFixMatch: 高置信度 hard CE，低置信度 sharpened soft KL。
    ...
    @staticmethod
    def get_argument():
        return ReFixMatch.get_argument() + ClosedFormIFRankMixin.ifrank_arguments()
