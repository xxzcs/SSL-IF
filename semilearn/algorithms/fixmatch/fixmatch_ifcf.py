# FixMatch + 闭式 last-layer TracIn 影响排序一致性 (纯 add; FixMatch 无关系项)。
# 不修改原 fixmatch.py。train_step 忠实复刻 FixMatch + 追加 ifrank_loss(软目标 mixin)。
import torch
...
@ALGORITHMS.register('fixmatch_ifcf')
class FixMatchIFcf(ClosedFormIFRankMixin, FixMatch):
    ...
    @staticmethod
    def get_argument():
        return FixMatch.get_argument() + ClosedFormIFRankMixin.ifrank_arguments()
