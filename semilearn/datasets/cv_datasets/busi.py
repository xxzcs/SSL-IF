...
mean, std = {}, {}
mean['busi'] = [0.330, 0.330, 0.330]
std['busi'] = [0.204, 0.204, 0.204]
...
def get_busi(args, alg, name, num_labels, num_classes, data_dir='../uda_data', lpath='', ulpath='', include_lb_to_ulb=False, val_path=''):
    ...
    transform_weak = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean['busi'], std['busi'])
    ])
    ...
    lb_dset = BUSDataset(root=os.path.join(data_dir, "trainbusi"), transform=transform_weak, ulb=False, alg=alg, db_path=lpath)

    ulb_dset = BUSDataset(root=os.path.join(data_dir, "trainbusi"), transform=transform_weak, alg=alg, ulb=True, medium_transform=transform_medium, strong_transform=transform_strong, 
                          db_path=ulpath, include_lb_to_ulb=include_lb_to_ulb) #, lb_index=lb_dset.lb_idx)

    val_full_path = os.path.join(data_dir, args.valdata)
    eval_dset = ImageFolder(val_full_path, transform=transform_val)

    return lb_dset, ulb_dset, eval_dset, None
