...
LOG=${LOG:-logs/bus_fixmatch_ifcf_ld1_soft_phase_${STAMP}.log}
BASE=${BASE:-config/usb_cv/fixmatch/fixmatch_bus_878_0.yaml}
SUMMARY=${SUMMARY:-results/fixmatch_ifcf_bus_ld1_softstrong_wu1510_latest.csv}
...
run_combo() {
  local warmup=$1
  local tag="fixmatch_ifcf_bus_ld1_soft_True_wu${warmup}"
...
echo "[$(date)] ===== BUS FixMatch+IFCF ld=1.0 soft-strong warmup1/5/10 start ====="
echo "base=${BASE}"
...
touch results/ALL_DONE_BUS_FIXMATCH_IFCF_LD1_SOFTSTRONG_WU1510
echo "[$(date)] ===== BUS FixMatch+IFCF ld=1.0 soft-strong warmup1/5/10 done ====="
