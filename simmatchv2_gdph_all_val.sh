#!/bin/bash
# SimMatchV2 GDPH 评测: 每比例(DA开) 评 best 和 latest
# 每折匹配 5 个 seed, 5 折共 25 个 checkpoint 直接聚合 -> mean±std (pooled-25, 与主表口径一致)
set -e
  ...
  for da in 1; do
    for kind in best latest; do
      ...
      python3 eval_sup_cv.py \
        --load_glob_template "saved_models/usb_cv/simmatchv2_gdph_${rtag}_da${da}_fold{fold}_*/${ck}" \
        --folds 0 1 2 3 4 \
        ...
        --summary_csv results/simmatchv2_gdph_cv.csv \
        --method_suffix ${kind} || echo "[warn] eval ${rtag} da${da} ${kind} 失败"
    done
  done
done
echo "===== GDPH SimMatchV2 最终汇总 ====="; cat results/simmatchv2_gdph_cv.csv
