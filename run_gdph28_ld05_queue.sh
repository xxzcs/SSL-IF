#!/usr/bin/env bash
# GDPH 20% queue with layer_decay fixed to 0.5 for BUS-compatible comparison.
# Default priority: FixMatch, FixMatch+IF, FlexMatch, FreeMatch, ReFixMatch, SoftMatch, AdaMatch.
# Use method fixmatch_if_soft for the same IF recipe with if_target=soft.
# Optional: include simmatch simmatchv2 in METHODS if the layer_decay audit needs them too.
set -u

cd /home/xiexiaozheng/Semi-supervised-learning

STAMP=$(date +%Y%m%d_%H%M%S)
LOG=${LOG:-logs/gdph28_ld05_${STAMP}.log}
SUMMARY=${SUMMARY:-results/gdph28_ld05_latest.csv}
...
METHODS=${METHODS:-"fixmatch fixmatch_if flexmatch freematch refixmatch softmatch adamatch"}
...
GDPH_LAYER_DECAY=0.5

# Locked BUS IF recipe: hard / strong / corrT0.9 / multiply_balanced / w1 / warmup5-zero /
...
IFPLUG_SOFT="ifrank_combine=multiply_balanced if_lambda=1 csim_lambda=1 num_references=4 ref_select=by_instance ref_cand_k=8 ifrank_loss_weight=1.0 if_target=soft use_strong_if=True corrT=0.9 ifrank_warmup_epochs=5 ifrank_warmup_mode=zero"
...
prepare_common_gdph28() {
  ...
  # GDPH uses label.csv fold + label_ratio; BUS-style split paths must be empty.
  setkv "$cfg" lpath "''"
  setkv "$cfg" ulpath "''"
}
...
base_config_for_method() {
  case "$1" in
    fixmatch|fixmatch_if|fixmatch_if_soft) echo "config/usb_cv/fixmatch/fixmatch_gdph_0.yaml" ;;
    ...
    simmatchv2) echo "config/usb_cv/simmatchv2/simmatchv2_gdph_28_0.yaml" ;;
    *) return 1 ;;
  esac
}
...
algorithm_for_method() {
  case "$1" in
    ...
    fixmatch_if|fixmatch_if_soft) echo "fixmatch_ifcf" ;;
...
launcher_for_method() {
  case "$1" in
    fixmatch_if|fixmatch_if_soft) echo "train_ifcf.py" ;;
    fixmatch|flexmatch|freematch|refixmatch|softmatch|adamatch|simmatch|simmatchv2) echo "train.py" ;;
    *) return 1 ;;
  esac
}
...
save_prefix_for_method() {
  case "$1" in
    fixmatch) echo "fixmatch_gdph_28_ld05" ;;
    fixmatch_if) echo "fixmatch_ifcf_gdph_28_ld05" ;;
    fixmatch_if_soft) echo "fixmatch_ifcf_soft_gdph_28_ld05" ;;
    flexmatch) echo "flexmatch_gdph_28_ld05" ;;
    freematch) echo "freematch_gdph_28_ld05" ;;
    refixmatch) echo "refixmatch_gdph_28_ld05" ;;
    softmatch) echo "softmatch_gdph_28_ld05" ;;
    adamatch) echo "adamatch_gdph_28_ld05" ;;
    simmatch) echo "simmatch_gdph_28_ld05" ;;
    simmatchv2) echo "simmatchv2_gdph_28_ld05" ;;
    *) return 1 ;;
  esac
}
...
run_one() {
  ...
  elif [ "$method" = "fixmatch_if_soft" ]; then
    for kv in $IFPLUG_SOFT; do
      setkv "$tmp" "${kv%%=*}" "${kv#*=}"
    done
  fi
...
echo "[$(date)] ===== GDPH 20% layer_decay=0.5 queue start ====="
...
touch results/ALL_DONE_GDPH28_LD05
echo "[$(date)] ===== GDPH 20% layer_decay=0.5 queue done: ${SUMMARY} ====="
