...
SUMMARY=${SUMMARY:-results/fixmatch_dif_only_bus_ld05_latest.csv}
STAMP=$(date +%Y%m%d_%H%M%S)
LOG=${LOG:-logs/fixmatch_dif_only_bus_ld05_${STAMP}.log}
...
echo "[$(date)] ===== BUS FixMatch+DIF-only ld0.5 start ====="
echo "base=${BASE}"
...
run_one() {
  local seed=$1
  local save_name="fixmatch_dif_only_bus_ld05_wu5_s${seed}"
  ...
  setkv "$tmp" ifrank_score_mode if
  setkv "$tmp" corrT 0.9
  setkv "$tmp" use_strong_if True
  setkv "$tmp" if_lambda 1
  setkv "$tmp" csim_lambda 0
...
eval_latest() {
  python3 eval_sup.py \
    ...
    --load_glob "saved_models/usb_cv/fixmatch_dif_only_bus_ld05_wu5_s*/latest_model.pth" \
    --method_suffix latest || echo "[$(date)] [warn] eval latest failed"
}

check_gpu || { echo "[$(date)] [error] no visible GPU; abort BUS FixMatch+DIF-only"; exit 1; }
...
touch results/ALL_DONE_BUS_FIXMATCH_DIF_ONLY_LD05
echo "[$(date)] ===== BUS FixMatch+DIF-only ld0.5 done ====="
