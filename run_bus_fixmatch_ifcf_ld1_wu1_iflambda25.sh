...
LOG=${LOG:-logs/bus_fixmatch_ifcf_ld1_wu1_iflambda25_${STAMP}.log}
BASE=${BASE:-config/usb_cv/fixmatch/fixmatch_bus_878_0.yaml}
SUMMARY=${SUMMARY:-results/fixmatch_ifcf_bus_ld1_wu1_iflambda25_latest.csv}
...
run_lambda() {
  local if_lambda=$1
  local tag="fixmatch_ifcf_bus_ld1_hard_True_wu1_w2_lam${if_lambda}"
...
echo "[$(date)] ===== BUS FixMatch+IFCF ld=1.0 hard-strong warmup1 weight2 if_lambda2/5 start ====="
echo "base=${BASE}"
...
touch results/ALL_DONE_BUS_FIXMATCH_IFCF_LD1_WU1_IFLAMBDA25
echo "[$(date)] ===== BUS FixMatch+IFCF ld=1.0 hard-strong warmup1 weight2 if_lambda2/5 done ====="
