#!/bin/bash
# USB SimMatch on GDPH: 0.1/0.2/0.3 (19/28/37) × DA开(da1) × 5折 × 5seed = 25/模型
# 健壮版: 不用 set -e; 跳过已完成; 单个 run 失败(如偶发段错误)重试一次, 仍失败则跳过继续
declare -A CFG=(
  [19]=config/usb_cv/simmatch/simmatch_gdph_19_0.yaml
  [28]=config/usb_cv/simmatch/simmatch_gdph_28_0.yaml
  [37]=config/usb_cv/simmatch/simmatch_gdph_37_0.yaml
)
...
for rtag in 19 28 37; do
  ...
  for fold in 0 1 2 3 4; do
    for seed in 1 2 3 4 5; do
      save_name="simmatch_gdph_${rtag}_da1_fold${fold}_${seed}"
